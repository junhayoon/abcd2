...
import template from './templates/FacilityCategory.html.js'
...
export default Vue.component('Facility', {
	...
	data: function () {
		return {
			...
			basicSymbolViewCount: 21,
			customSymbolViewCount: 14,
			symbolViewCount: 6
		}
	},
	computed: {
		...
		filteredFacilityItems: function() {
			if(this.currentCategoryFilter.fcode=='-1') {
				return this.facilityItems;
			} else {
				return this.facilityItems.filter(item => {
					return item.facilityCategory.fcode == this.currentCategoryFilter.fcode;
	...
	watch: {
		customesymbolitems: function() {
			...
			// 항상 7의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 7의 배수만큼 칸을 지정해줌
			if(this.customSymbolViewCount < this.customesymbolitems.length) {
				this.customSymbolViewCount = (parseInt(this.customesymbolitems.length / 7) + 1) * 7;
			}
		},
		basicsymbolitems: function() {
			...
			// 항상 7의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 7의 배수만큼 칸을 지정해줌
			if(this.basicSymbolViewCount < this.basicsymbolitems.length) {
				this.basicSymbolViewCount = (parseInt(this.basicsymbolitems.length / 7) + 1) * 7;
			}
		},
		symbolItems: function() {
			// 항상 3의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 3의 배수만큼 칸을 지정해줌
			if(this.symbolViewCount < this.symbolItems.length) {
				this.symbolViewCount = (parseInt(this.symbolItems.length / 3) + 1) * 3;
			}
		},
	...
	methods: {
		myTest: function() {
			this.basicsymbolitems.push({
				title: "air-sensor.png".split('.').slice(0, -1).join('.'),
				path: this.apiSymbolFile +"/" + "air-sensor.png" + '?removable=0',
				removable: false 
			});
		},
    		...
    		.then(res => {
    			if(res != null && res.data != null) {
    				...
    				for(var i=0; i < obj.length; i++ ) {
    					if(mode == 0) {
    						// 항상 7의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 7의 배수만큼 칸을 지정해줌
//    						if(me.basicSymbolViewCount < obj.length) {
//    							me.basicSymbolViewCount = (parseInt(obj.length / 7) + 1) * 7;
//    						}
        					...
        					// 항상 7의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 7의 배수만큼 칸을 지정해줌
//        					if(me.customSymbolViewCount < obj.length) {
//    							me.customSymbolViewCount = (parseInt(obj.length / 7) + 1) * 7;
//    						}
			...
			if(files != null) {
				...
				.then(res => {
    				...
    				for(var i=0; i < obj.length; i++ ) {//###################################################################
    					...
    					// 항상 7의 배수로 심볼 칸을 보여주기 위해서 심볼 갯수를 체크하여 7의 배수만큼 칸을 지정해줌
//    					var customLen = me.customesymbolitems.length;
//    					if(me.customSymbolViewCount < customLen) {
//							me.customSymbolViewCount = (parseInt(customLen / 7) + 1) * 7;
//						}
        			}
		...
		onSelectFacility: function(index) {
			this.$refs['viewMiniMap'].selectedSymbol(this.filteredFacilityItems[index].ccode);
		...
		onViewMiniMapSeletedSymbol: function(id) {
			for(var i=0; i<this.filteredFacilityItems.length; i++) {
				if(this.filteredFacilityItems[i].ccode == id) {
		...
		onConfirmCategoryDeleteOk: function(e) {
			...
			axios.delete(this.apiCategory + "/" + target.ccode)
			.then(res => {
				me.init();
	        })
	        .catch(err => {
		...
		subStyle: function() {
			return 'background: url("/static/images/facility_category_bg.jpg") no-repeat; background-size: 100% 100%;';
		},
	...
	mounted: function () {
		...
		me.categoryEnableSelector = new tui.SelectBox('#categoryEnable', {
			data: me.enabledData,
			autofocus: true,
			showIcon: true
		});
