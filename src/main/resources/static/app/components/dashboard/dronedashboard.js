...
import template from '../templates/dashboard/dronedashboard.html.js'
import i18n from '../../i18n/index.js'

export default Vue.component('dronedashboard', {
    i18n,
    props: {
        selectedFacility: Object,
        isOpenDroneStation: Boolean,
        isShowPtzCtrl: Boolean,
        droneStationContainerId: String,
        parentContainerId: String,
        isFullScreen: Boolean,
        droneStationFacility: Object
    },
    template: template,
    data: function () {
        return {
    ...
    methods: {
        activeSelectFacility: function() {
            $("#" + this.droneStationContainerId).find("li").removeClass("border border-warning bg-dark text-white");
            $("#" + this.droneStationContainerId).find("li").css("margin", "1px");
        ...
        init: function() {
            ...
            // 현재 열린 드론의 영상관련 정보가 있다면 영상 재생
            var droneStationFacility = me.droneStationFacility;
            if(droneStationFacility[0].properties != null && droneStationFacility[0].properties.mediaIp != null 
                && droneStationFacility[0].properties.mediaPort != null && droneStationFacility[0].properties.mediaVurixUrl != null
                && droneStationFacility[0].properties.mediaIp.length > 0 && droneStationFacility[0].properties.mediaVurixUrl.length > 0) {
        ...
        playVmsVideo: function () {
            ...
            setTimeout(() => {
                var playFacility = me.droneStationFacility;
                let elementId = "dron_streaming";
                let elementPlayer = document.getElementById(elementId);
                
                if (elementPlayer != null) {
                    let player = me.createVmsPlayer(elementId, playFacility[0].properties.mediaIp, playFacility[0].properties.mediaPort, playFacility[0].properties.mediaVurixUrl);
                    if (player != null) {
        ...
        vmsMsgCallback: function(err) {
            ...
            console.log("dronedashboard.js vmsMsgCallback() end..." + errStr);
        },
    ...
    beforeMounted: function() {
        //console.log("dronedashboard.js beforeMounted: end...");
    },
    mounted: function () {
        let me = this;
        
        if(this.isOpenDroneStation) {
            me.init();
        }
    ...
    updated: function() {
        ...
        //console.log("dronedashboard.js updated end...");                                   
    },
    beforeDestroy: function() {
        this.clearVmsPlayer();
        //console.log("dronedashboard.js beforeDestroy end...");
    },
});